...
<section class="dash_content_app">
   ...
   <div class="dash_content_app_box">
      <div class="nav">
         ...
         @if(! isset($user))
            <form action="{{ route('root.users.store') }}" method="post" class="app_form"
               enctype="multipart/form-data">
            @else
               <form
                  action="{{ route('root.users.update', ['user' => $user->id]) }}"
                  ...
                  <label class="label">
                     ...
                     <select name="company_id" class="select2">
                        ...
                        @foreach($companies as $company)
                           <option value="{{ $company->id}}" {{ $company->id == old('company_id', $user->company_id ?? null) ? 'selected' : '' }}>
