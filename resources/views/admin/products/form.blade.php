...
<section class="dash_content_app">
   ...
   <div class="dash_content_app_box">
      ...
      <div class="nav">
         ...
         @if(! isset($product))
            <form action="{{ route('inventories.store') }}" method="post" class="app_form"
               enctype="multipart/form-data">
            @else
               <form
                  action="{{ route('inventories.update', ['inventory' => $product->id]) }}"
                     ...
                     <select name="category_id" class="select2">
                        ...
                        @foreach($categories as $category)
                           <option value="{{ $category->id }}"
                              {{ set_selected($category->id, old('category_id', $product->category_id ??  null)) }}>
